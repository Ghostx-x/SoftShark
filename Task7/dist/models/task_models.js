...
export class TaskModel {
    ...
    async list(project_id, status, due_date) {
        ...
        if (status)
            query.andWhere('task.status = :status', { status: status.toLowerCase().trim() });
        if (due_date)
            query.andWhere('task.due_date = :due_date', { due_date });
        return await query
    ...
    async updateAssign(task_id, new_assigned_to) {
